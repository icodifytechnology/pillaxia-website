...
function Pill({ children }: { children: React.ReactNode }) {
  return (
    <span
      className="inline-flex items-center gap-1.5 rounded-full px-4 py-1.5 text-[11px] font-bold tracking-[0.2em] uppercase"
      style={{
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-1">
        <time className="text-[11px] text-slate-500 font-medium mb-3">
          {date}
        </time>
        ...
        <span
          className="inline-flex items-center gap-1 mt-5 text-sm font-bold"
          style={{ color: ACCENT }}
          aria-hidden="true"
        >
          Read more
  ...
  return (
    ...
    >
      <div
        ...
        aria-hidden="true"
        style={{
          background: `linear-gradient(to bottom, ${ACCENT}, ${MAGENTA})`,
        }}
      />
  ...
  return (
    <>
      ...
      <aside
        role="dialog"
        aria-modal="true"
        aria-label={isEvent ? ev!.title : pr!.title}
      ...
      >
        <div
          className="sticky top-0 z-10 flex items-start justify-between gap-4 p-6"
          style={{
            background: '#07101f',
        ...
        <div className="p-6 space-y-6">
          {(isEvent ? ev?.image : pr?.image) && (
            ...
            >
              <Image
                src={isEvent ? ev!.image! : pr!.image!}
                alt={isEvent ? ev!.title : pr!.title}
                fill
          ...
          {!isEvent && pr?.date && (
            <time className="text-xs text-slate-500 font-medium -mt-2 block">
              {pr.date}
            </time>
          )}

          <p className="text-slate-300 leading-relaxed text-sm whitespace-pre-line">
            {isEvent ? ev!.fullDescription : (pr as PressRelease).fullContent}
          </p>
          ...
          {isEvent
            ...
            : (
                <a
                  href={(pr as PressRelease | NewsItem).url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-full font-bold text-sm transition-all hover:gap-4"
                  style={{
...
function EmptyState({ label }: { label: string }) {
  return (
    <div
      className="col-span-2 text-center py-20 rounded-3xl"
      style={{
        background: 'rgba(255,255,255,0.03)',
        border: '1px solid rgba(255,255,255,0.08)',
      }}
...
export function PressPage() {
  ...
  return (
    <main className="overflow-x-hidden">
      {/* ── HERO ── */}
      <section
        className="relative min-h-[60vh] flex items-center pt-32 pb-20 overflow-hidden mesh-dark"
      ...
      >
        ...
        <div className="container mx-auto px-6 md:px-12 relative z-10">
          <Reveal className="flex flex-col items-center text-center space-y-7 max-w-3xl mx-auto">
            <Pill>
              <NewspaperIcon size={11} aria-hidden="true" /> Newsroom
            </Pill>
            ...
            <p className="text-lg leading-relaxed text-slate-300 max-w-xl">
              The latest announcements, news coverage, and events from
              Pillaxia — the AI-powered connected care platform for chronic
              health management.
            </p>
            ...
            <div
              role="tablist"
              aria-label="Press content categories"
      ...
      <section className="relative py-24 overflow-hidden mesh-dark">
        ...
        <div className="container mx-auto px-6 md:px-12 relative z-10 max-w-5xl">
          ...
          {/* 
            SEO: All three tab panels are always in the DOM.
            Only the active one is visible. Crawlers see all content
            regardless of which tab is "active" in JS.
          */}
          <div
            role="tabpanel"
            id="tabpanel-events"
                ...
                EVENTS.map((ev, i) => (
                  <Reveal key={ev.id} delay={i * 80}>
                    <EventCard
                      event={ev}
                      onClick={() =>
                        setSelected({ kind: 'event', data: ev })
                      }
                    />
                  </Reveal>
                ))
          ...
          <div
            ...
            className={activeTab === 'Press Releases' ? '' : 'hidden'}
          >
            <div className="grid md:grid-cols-2 gap-6">
                ...
                PRESS_RELEASES.map((pr, i) => (
                  <Reveal key={pr.id} delay={i * 80}>
                    <PressCard
                      ...
                      image={pr.image}
                      onClick={() =>
                        setSelected({ kind: 'press', data: pr })
                      }
                    />
                  </Reveal>
                ))
          ...
          <div
            role="tabpanel"
            id="tabpanel-news-&-coverage"
            aria-label="News & Coverage"
            className={activeTab === 'News & Coverage' ? '' : 'hidden'}
          >
            <div className="grid md:grid-cols-2 gap-6">
              {NEWS_ITEMS.length > 0 ? (
                NEWS_ITEMS.map((n, i) => (
